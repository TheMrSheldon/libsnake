...
namespace ls {
	...
	public:
		 ...
		 * If the game is not over #SnakeFlags::None is returned.
		 * 
		 ...
		 * dead ones) such that the move of the snake at index i in #State::getSnakes() is at index i
		 * in \p moves.
		 * 
		 * @param state The state which should be stepped. The state itself will not be modified.
		 * @param moves The moves of each snake at the corresponding position in #State::getSnakes().
		 * 	As such \p moves should have exactly the same number of arguments as #State::getSnakes().
